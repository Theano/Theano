...
class BadOptimization(DebugModeError):
    ...
    def str_diagnostic(self):
        ...
        try:
            ...
            #      nv == ov == 0
            reldiff = (abs_diff /
                       np.maaximum(np.absolute(nv) + np.absolute(ov),
                                      1e-8))
...
def _lessbroken_deepcopy(a):
    ...
    object
        ...
        (a deep copy). This function handles np arrays specially, because
        copy.deepcopy() called on a 0-d array will return a np scalar,
        not an array.

    """
    # this exists because copy.deepcopy on np arrays is broken
