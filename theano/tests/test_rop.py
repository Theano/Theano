...
class test_RopLop(RopLop_checker):
    ...
    def test_conv(self):
        for conv_op in [conv.conv2d, conv2d]:
            for border_mode in ['valid', 'full']:
                ...
                filters = tensor.TensorType(theano.config.floatX,
                                            [False] * filter_dim)(name='filter')
                ev_input = tensor.TensorType(theano.config.floatX,
    ...
    def test_alloc(self):
        ...
        out3d = tensor.alloc(self.x, self.mat_in_shape[0],
                             self.mat_in_shape[1], self.in_shape[0])
