...
def run_conv_nnet2(use_gpu):  # pretend we are training LeNet for MNIST
    ...
    conv_op1 = conv.ConvOp((n_kern, logical_hid_shape[0] // 2,
                            logical_hid_shape[1] // 2), shape_kern1[2:],
                           n_kern1, n_batch, 1, 1)
...
def cmp_run_conv_nnet2_classif(seed, isize, ksize, bsize,
    ...
    try:
        ...
        utt.seed_rng(seed)  # Seeds numpy.random with seed
        train_cpu, params_cpu, x_shape, y_shape, mode_cpu = build_conv_nnet2_classif(
            use_gpu=False,
            isize=isize,
            ksize=ksize,
            n_batch=bsize,
        ...
        utt.seed_rng(seed)  # Seeds numpy.random with seed
        train_gpu, params_gpu, x_shape_gpu, y_shape_gpu, mode_gpu = build_conv_nnet2_classif(
            use_gpu=True,
            isize=isize,
            ksize=ksize,
            n_batch=bsize,
