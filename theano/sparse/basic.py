...
class _sparse_py_operators:
    ...
    def __getitem__(self, index): 
        if isinstance(index, tuple) and all([not isinstance(idx, slice) for idx in index]):
            return getitem_scalar(self, index)
        elif not isinstance(index, tuple):            
            return getitem2d(self, (index,))
        else:            
            return getitem2d(self, index)
...
class GetItem2d(gof.op.Op):
    ...
    def make_node(self, x, index):
        ...
        if len(index)==1:
            ...
            inp += [i, i]
        return gof.Apply(self, inp, [x.type()])
...
class GetItemScalar(gof.op.Op):
    ...
    def make_node(self, x, index):
        ...
        return gof.Apply(self, inp, [tensor.scalar(dtype=x.dtype)])
    def perform(self, node, (x, idx1, idx2), (out, )):
        assert _is_sparse(x)
        out[0] = numpy.asarray(x[idx1, idx2])

getitem_scalar = GetItemScalar()
