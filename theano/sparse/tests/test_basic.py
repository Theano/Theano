...
def sparse_random_inputs(format, shape, n=1, out_dtype=None, p=0.5, gap=None,
    ...
    # np 1.5.0 with scipy 0.9.0 have scipy.sparse.XXX_matrix return
    ...
    def test_upcast(self):
        ...
        for dense_dtype in typenames:
            for sparse_dtype in typenames:
                ...
                # print 'np dtype num', mat.dtype.num
...
class DotTests(utt.InferShapeTester):
    ...
    def test_cuda(self):
        ...
        d_theano = f(a_val)
        d_np = a_val * b.get_value()
        utt.assert_allclose(d_np, d_theano)
...
def test_size():
    """
    Ensure the `size` attribute of sparse matrices behaves as in np.
    """
...
class Remove0Tester(utt.InferShapeTester):
    ...
    def test_remove0(self):
        configs = [
            # structure type, np matching class
...
class Test_getitem(unittest.TestCase):
    ...
    def test_GetItem2D(self):
        ...
        for format in sparse_formats:
            ...
            deviation from np indexing rule. This decision is made largely
            for keeping the consistency between np and theano.
    ...
    class Tester(unittest.TestCase):
        ...
        def test_op(self):
            for format in sparse.sparse_formats:
                ...
                # First, in recent version of np, when a np
