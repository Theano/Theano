...
class TensorType(Type):
    ...
    ----------
    dtype: str
        Corresponding to np dtype (e.g., 'int64')
                ...
                if isinstance(data, np.ndarray):
                    ...
                    if TensorType.values_eq(np.asarray(data),
                        ...
                        err_msg = (
                            ...
                            '1) explicitly convert your data to a np array '
                            'of dtype %s, or '
                            '2) set "allow_input_downcast=True" when calling '
                            '"function".'
    ...
    def values_eq_approx(a, b, allow_remove_inf=False, allow_remove_nan=False,
        ...
        if isinstance(a, np.ndarray) and isinstance(b, np.ndarray):
            ...
            if 'int' in str(a.dtype):
                ...
                # work around a np.allclose bug:
                # http://projects.scipy.org/numpy/ticket/1672
                if a.ndim == 0 and np.isinf(a):
                    a = a.reshape(1)
                    b = b.reshape(1)
                ...
                if not (a_missing.any() or (allow_remove_inf and a_inf.any())):
                    ...
                    # reason why np.allclose(a, b) returned False.
                    _logger.info(
                        'np allclose failed for abs_err %f and rel_err %f',
                        np.max(abs(a - b)),
                        np.max(abs(a - b) / (abs(a) + abs(b))))
                    return False
                # The following line is what np.allclose bases its decision
    ...
    def value_zeros(self, shape):
        """
        Create an np ndarray full of 0 values.
