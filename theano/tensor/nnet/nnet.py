...
class SoftmaxWithBias(gof.Op):
    ...
    def perform(self, node, input_storage, output_storage):
        ...
        # for i in xrange(sm.shape[0]):
            # row = x[i] + b
            # sm[i] = np.exp(row - np.max(row))
            # sm[i] *= 1.0 / np.sum(sm[i])
...
def _check_rows_is_arange_len_labels(rows, labels):
    ...
    if labels.owner and hasattr(labels.owner.fgraph, 'shape_feature'):
        ...
        # _is_const(rows, np.arange(...)) does not work for the moment.
...
def relu(x, alpha=0):
    ...
    if alpha == 0:
        ...
        # np dtype, they will be considered as float64, so would
