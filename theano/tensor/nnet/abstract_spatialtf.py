...
class AbstractSpatialTransformerOp(Op):
    def make_node(img, grid):
        raise NotImplementedError('make_node for AbstractSpatialTf is not implemented.')


class SpatialTransformerOp(AbstractSpatialTransformerOp):
    def __init__(self, border_mode):
        self.border_mode = border_mode

    def make_node(self, inp, theta, out_dims):
        _inp = as_tensor_variable(inp)
        _theta = as_tensor_variable(theta)
        _out_dims = as_tensor_variable(out_dims)
        ...
        return Apply(self, [_inp, _theta, _out_dims], [out])

    def perform(self, node, inp, out):
        inputs, theta, out_dims = inp
        # Assumes tensor is in NCHW format
        num_batch, num_channels, height, width = inputs.shape
        theta = T.reshape(theta, (-1, 2, 3))

        # grid of (x_t, y_t, 1), eq (1) in ref [1]
        scale_height, scale_width = out_dims[2:]
        ...
        inputs_dim = inputs.dimshuffle(0, 2, 3, 1)
        inputs_transformed = self._interpolate(inputs_dim, x_s_flat, y_s_flat,
                                               out_height, out_width, self.border_mode)
...
def spatialtf(inp, theta, scale_height=1, scale_width=1, border_mode='nearest'):
    out_dims = (inp.shape[0], inp.shape[1],
                T.cast(T.ceil(inp.shape[1] * scale_height), 'int64'),
                T.cast(T.ceil(inp.shape[2] * scale_width), 'int64'))

    return SpatialTransformerOp(border_mode)(inp, theta, out_dims)
