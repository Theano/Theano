...
class DummyConvGrad3D:
    ...
    def __init__(self, rng, VdHvals, d, WShape):
        ...
        self.dV = shared(rng.uniform(-1, 1,
                                     self.V.get_value(borrow=True).shape))
        self.ddCdH = shared(
            rng.uniform(-1, 1, self.dCdH.get_value(borrow=True).shape))
        self.d = d
        self.WShape = WShape
...
class TestConv3D(utt.InferShapeTester):
    ...
    def setUp(self):
        ...
        self.dCdW_shape_func = function(
            [self.RShape], T.grad(self.reconsObj, self.W).shape, mode=mode)
    ...
    def test_c_against_mat_mul(self):
        ...
        self.W.set_value(self.random_tensor(
            numFilters, filterHeight, filterWidth, filterDur, inputChannels),
            borrow=True)
        self.W.set_value(
        ...
        self.V.set_value(
            self.random_tensor(
                ...
                videoWidth, videoDur, inputChannels),
            borrow=True)
    ...
    def test_c_against_mat_transp_mul(self):
        ...
        self.W.set_value(
            self.random_tensor(
                numFilters, filterHeight,
                filterWidth, filterDur, inputChannels),
            borrow=True)
    ...
    def test_c_against_sparse_mat_transp_mul(self):
        ...
        videoDur = (
            time_steps - 1) * dt + filterDur + \
            self.rng.randint(0, 3)
        videoWidth = (
            col_steps - 1) * dc + filterWidth + \
            self.rng.randint(0, 3)
        videoHeight = (
            row_steps - 1) * dr + filterHeight + \
            self.rng.randint(0, 3)
    ...
    def test_infer_shape(self):
        ...
        self._compile_and_check(
            [self.RShape], [self.R], [self.V.get_value(borrow=True).shape[1:4]],
            ConvTransp3D)
    ...
    def test_gradient(self):
        ...
        theano.tests.unittest_tools.verify_grad(
            DummyConv3D(rng, (V, W, b), d),
            [0.0], n_tests=testsPerDir)
        theano.tests.unittest_tools.verify_grad(
            DummyConvTransp3D(
                rng, (W, rb, dCdH), d, V.get_value(borrow=True).shape[1:4]),
            [0.0], n_tests=testsPerDir)
        theano.tests.unittest_tools.verify_grad(DummyConvGrad3D(
            rng, (V, dCdH),
