...
#([y*]exp(x**2))/erfc(-x) => [y*](when x>threashold, sqrt(pi)*-x/(1-1/(2*x**2)+3/(4*x**4)-15/(8*x**6)))
...
def local_elemwise_fusion_op(OP, max_input_fct=lambda node: 1024):
    ...
    :param max_input_fct: a function that returns the maximum number of inputs
                          ...
                          resulting fused op. This is big enough that
    ...
    def local_fuse(node):
        ...
        s_g=[] # Inputs of the new scalar op that represents the current node.
        ...
        assert new_nb_input == len(inputs)
