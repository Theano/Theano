    ...
    def with_linker(self, linker, scalar_op=scalar.add, dtype="floatX",
        ...
        for xsh, tosum in self.cases:
            ...
            if tensor_op == tensor.all:
                ...
                for axis in reversed(sorted(tosum)):
                    zv = np.bitwise_and.reduce(zv, axis)
                ...
                raise Exception(
                    ...
                    str(scalar_op))
            if scalar_op in [scalar.maximum, scalar.minimum] and np_raised:
                try:
                    out = f(xv)
...
class test_Prod(unittest.TestCase):
    ...
    def test_other_grad_tests(self):
        ...
        assert np.allclose(grad_fn(x_val1), [[6., 3., 2.], [30., 0.,
            0.], [0., 0., 0.]])
        assert np.allclose(grad_fn(x_val2), [[0., 0., 2.], [30.,
             0., 0.], [72., 63., 56.], [0., 0., 90.]])

        p_axis0 = Prod(axis=0)
        grad_p_axis0 = theano.tensor.grad(p_axis0(x).sum(), x)
        grad_fn_axis0 = theano.function([x], grad_p_axis0, mode=self.mode)
        assert np.allclose(grad_fn_axis0(x_val2), [[0., 400.,
