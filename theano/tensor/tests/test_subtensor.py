...
class T_subtensor(unittest.TestCase, utt.TestOptimizationMixin):
    ...
    def test_ellipsis(self):
        np_n = np.arange(24, dtype=self.dtype).reshape((2, 3, 4))
        n = self.shared(np_n)
        test_cases = [
        ...
        for length, op_type, op_type_opt, slice_ in test_cases:
            ...
            tval = self.eval_output_and_check(t,
                                              op_type=op_type_opt,
                                              length=length)
            assert_equal(tval.shape, np_tval.shape)
            assert_array_equal(tval, np_tval)
    ...
    def test_advanced1_inc_and_set(self):
        ...
        for f_out, output_num in izip(f_outs, all_outputs_num):
            ...
            assert np.allclose(f_out, output_num)
    ...
    def test_adv1_inc_sub_notlastdim_1_2dval_no_broadcast(self):
        ...
        try:
            ...
            for i, shp_i, shp_v in zip(sym_i, shape_i, shape_val):
                ...
                # not as a list or array, np only increments the indexed
