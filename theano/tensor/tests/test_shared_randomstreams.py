...
class T_SharedRandomStreams(unittest.TestCase):
    ...
    def test_basics(self):
        ...
        # print fn_val0
        np_val0 = rng.uniform(size=(2, 2))
        np_val1 = rng.uniform(size=(2, 2))
        # print np_val0

        assert np.allclose(fn_val0, np_val0)
        print(fn_val0)
        print(np_val0)
        print(fn_val1)
        print(np_val1)
        assert np.allclose(fn_val1, np_val1)
    ...
    def test_uniform(self):
        """Test that RandomStreams.uniform generates the same results as np"""
        ...
        rng_seed = np.random.RandomState(utt.fetch_seed()).randint(2**30)
        rng = np.random.RandomState(int(rng_seed))  # int() is for 32bit
        np_val0 = rng.uniform(-1, 1, size=(2, 2))
        np_val1 = rng.uniform(-1, 1, size=(2, 2))

        assert np.allclose(fn_val0, np_val0)
        assert np.allclose(fn_val1, np_val1)

    def test_normal(self):
        """Test that RandomStreams.normal generates the same results as np"""
        # Check over two calls to see if the random state is correctly updated.
        ...
        rng_seed = np.random.RandomState(utt.fetch_seed()).randint(2**30)
        rng = np.random.RandomState(int(rng_seed))  # int() is for 32bit
        np_val0 = rng.normal(-1, 2, size=(2, 2))
        np_val1 = rng.normal(-1, 2, size=(2, 2))
    ...
    def test_random_integers(self):
        ...
        # results as np.  We use randint() for np since
        ...
        rng_seed = np.random.RandomState(utt.fetch_seed()).randint(2**30)
        rng = np.random.RandomState(int(rng_seed))  # int() is for 32bit
        np_val0 = rng.randint(-5, 6, size=(20, 20))
        np_val1 = rng.randint(-5, 6, size=(20, 20))

        assert np.all(fn_val0 == np_val0)
        assert np.all(fn_val1 == np_val1)
    
    def test_choice(self):
        """Test that RandomStreams.choice generates the same results as np"""
        # np.random.choice is only available for np versions >= 1.7
        major, minor, _ = np.version.short_version.split('.')
        ...
        rng_seed = np.random.RandomState(utt.fetch_seed()).randint(2**30)
        rng = np.random.RandomState(int(rng_seed))  # int() is for 32bit
        np_val0 = rng.choice(10, (11, 8), True, None)
        np_val1 = rng.choice(10, (11, 8), True, None)

        assert np.all(fn_val0 == np_val0)
        assert np.all(fn_val1 == np_val1)

    def test_poisson(self):
        """Test that RandomStreams.poisson generates the same results as np"""
        ...
        rng_seed = np.random.RandomState(utt.fetch_seed()).randint(2**30)
        rng = np.random.RandomState(int(rng_seed))  # int() is for 32bit
        np_val0 = rng.poisson(lam=5, size=(11, 8))
        np_val1 = rng.poisson(lam=5, size=(11, 8))

        assert np.all(fn_val0 == np_val0)
        assert np.all(fn_val1 == np_val1)

    def test_permutation(self):
        """Test that RandomStreams.permutation generates the same results as np"""
        # Check over two calls to see if the random state is correctly updated.
        random = RandomStreams(utt.fetch_seed())
        ...
        np_val0 = np.asarray([rng.permutation(10) for i in range(20)])
        np_val1 = np.asarray([rng.permutation(10) for i in range(20)])

        assert np.all(fn_val0 == np_val0)
        assert np.all(fn_val1 == np_val1)

    def test_multinomial(self):
        """Test that RandomStreams.multinomial generates the same results as np"""
        # Check over two calls to see if the random state is correctly updated.
        random = RandomStreams(utt.fetch_seed())
        ...
        rng_seed = np.random.RandomState(utt.fetch_seed()).randint(2**30)
        rng = np.random.RandomState(int(rng_seed))  # int() is for 32bit
        np_val0 = rng.multinomial(1, [0.1]*10, size=(4, 4))
        np_val1 = rng.multinomial(1, [0.1]*10, size=(4, 4))
    ...
    def test_shuffle_row_elements(self):
        ...
        np_mval0 = in_mval.copy()
        np_mval1 = in_mval.copy()
        for row in np_mval0:
            rng.shuffle(row)
        for row in np_mval1:
            rng.shuffle(row)

        assert np.all(np_mval0 == fn_mval0)
        assert np.all(np_mval1 == fn_mval1)
        ...
        fn_vval = f1(in_vval)
        np_vval = in_vval.copy()
        vrng = np.random.RandomState(int(rng_seed))
        vrng.shuffle(np_vval)
        print(in_vval)
        print(fn_vval)
        print(np_vval)
        assert np.all(np_vval == fn_vval)
    ...
    def test_default_shape(self):
        ...
        val0 = f()
        val1 = f()
        np_val0 = uniform_rng.uniform()
        np_val1 = uniform_rng.uniform()
        assert np.allclose(val0, np_val0)
        assert np.allclose(val1, np_val1)

        for i in range(10):  # every test has 50% chance of passing even with non-matching random states
            val2 = g()
            np_val2 = multinomial_rng.multinomial(n=1, pvals=[.5, .5])
            assert np.all(val2 == np_val2)
    ...
    def test_vector_arguments(self):
        ...
        np_rng = np.random.RandomState(int(seed_gen.randint(2**30)))
        val0 = f([-5, .5, 0, 1])
        val1 = f([.9])
        np_val0 = np_rng.uniform(low=[-5, .5, 0, 1], high=1)
        np_val1 = np_rng.uniform(low=[.9], high=1)
        assert np.all(val0 == np_val0)
        assert np.all(val1 == np_val1)
        ...
        val0b = fb([-4., -2], [-1, 0])
        val1b = fb([-4.], [-1])
        np_val0b = np_rng.uniform(low=[-4., -2], high=[-1, 0])
        np_val1b = np_rng.uniform(low=[-4.], high=[-1])
        assert np.all(val0b == np_val0b)
        assert np.all(val1b == np_val1b)
        ...
        val0c = fc([-4., -2], [-1, 0], [2])
        val1c = fc([-4.], [-1], [1])
        np_val0c = np_rng.uniform(low=[-4., -2], high=[-1, 0])
        np_val1c = np_rng.uniform(low=[-4.], high=[-1])
        assert np.all(val0c == np_val0c)
        assert np.all(val1c == np_val1c)
    ...
    def test_broadcast_arguments(self):
        ...
        rng_seed = np.random.RandomState(utt.fetch_seed()).randint(2**30)
        np_rng = np.random.RandomState(int(rng_seed))
        val0 = f([-5, .5, 0, 1], [[1.]])
        val1 = f([.9], [[1.], [1.1], [1.5]])
        val2 = f([-5, .5, 0, 1], [[1.], [1.1], [1.5]])

        np_val0 = np_rng.uniform(low=[-5, .5, 0, 1], high=[1.])
        np_val1 = np_rng.uniform(low=[.9], high=[[1.], [1.1], [1.5]])
        np_val2 = np_rng.uniform(low=[-5, .5, 0, 1], high=[[1.], [1.1], [1.5]])
    ...
    def test_uniform_vector(self):
        ...
        np_val0 = np_rng.uniform(low=low_val, high=high_val)
        print('THEANO', val0)
        print('NUMPY', np_val0)
        assert np.all(val0 == np_val0)
        ...
        np_val1 = np_rng.uniform(low=low_val[:-1], high=high_val[:-1])
        print('THEANO', val1)
        print('NUMPY', np_val1)
        assert np.all(val1 == np_val1)
        ...
        np_rng = np.random.RandomState(int(seed_gen.randint(2**30)))
        np_val2 = np_rng.uniform(low=low_val, high=high_val, size=(3,))
        assert np.all(val2 == np_val2)
    ...
    def test_binomial_vector(self):
        ...
        np_val0 = np_rng.binomial(n=n_val, p=prob_val)
        assert np.all(val0 == np_val0)

        # arguments of size (2,)
        val1 = f(n_val[:-1], prob_val[:-1])
        np_val1 = np_rng.binomial(n=n_val[:-1], p=prob_val[:-1])
        assert np.all(val1 == np_val1)
        ...
        np_rng = np.random.RandomState(int(seed_gen.randint(2**30)))
        np_val2 = np_rng.binomial(n=n_val, p=prob_val, size=(3,))
        assert np.all(val2 == np_val2)
    ...
    def test_normal_vector(self):
        ...
        np_val0 = np_rng.normal(loc=avg_val, scale=std_val)
        assert np.allclose(val0, np_val0)

        # arguments of size (2,)
        val1 = f(avg_val[:-1], std_val[:-1])
        np_val1 = np_rng.normal(loc=avg_val[:-1], scale=std_val[:-1])
        assert np.allclose(val1, np_val1)
        ...
        np_rng = np.random.RandomState(int(seed_gen.randint(2**30)))
        np_val2 = np_rng.normal(loc=avg_val, scale=std_val, size=(3,))
        assert np.allclose(val2, np_val2)
    ...
    def test_random_integers_vector(self):
        ...
        # Arguments of size (3,)
        val0 = f(low_val, high_val)
        np_val0 = np.asarray([np_rng.randint(low=lv, high=hv+1)
            for lv, hv in zip(low_val, high_val)])
        assert np.all(val0 == np_val0)

        # arguments of size (2,)
        val1 = f(low_val[:-1], high_val[:-1])
        np_val1 = np.asarray([np_rng.randint(low=lv, high=hv+1)
        ...
        val2 = g(low_val, high_val)
        np_rng = np.random.RandomState(int(seed_gen.randint(2**30)))
        np_val2 = np.asarray([np_rng.randint(low=lv, high=hv+1)
            for lv, hv in zip(low_val, high_val)])
        assert np.all(val2 == np_val2)
        self.assertRaises(ValueError, g, low_val[:-1], high_val[:-1])
    ...
    def test_multinomial_vector(self):
        ...
        np_val0 = np.asarray([np_rng.multinomial(n=nv, pvals=pv)
            for nv, pv in zip(n_val, pvals_val)])
        assert np.all(val0 == np_val0)

        # arguments of size (2,)
        val1 = f(n_val[:-1], pvals_val[:-1])
        np_val1 = np.asarray([np_rng.multinomial(n=nv, pvals=pv)
        ...
        np_rng = np.random.RandomState(int(seed_gen.randint(2**30)))
        np_val2 = np.asarray([np_rng.multinomial(n=nv, pvals=pv)
            for nv, pv in zip(n_val, pvals_val)])
        assert np.all(val2 == np_val2)
