...
def get_numeric_subclasses(cls=np.number, ignore=None):
    """
    Return subclasses of `cls` in the np scalar hierarchy.
...
def get_numeric_types(with_int=True, with_float=True, with_complex=False,
    ...
    :param only_theano_types: If True, then np numeric data types that are
    ...
    automatically detect new data types introduced in np.
    """
        ...
        def test_good(self):
            ...
            for testname, inputs in iteritems(good):
                ...
                if (isinstance(self.expected, dict)
                    ...
                    # with np version, when we print a number and read it
                ...
                for i, (variable, expected) in enumerate(
                    ...
                    if (variable.dtype != expected.dtype
                            ...
                            or not np.allclose(variable, expected,
                                atol=eps, rtol=eps)):
...
def rand_nonzero(shape, eps=3e-4):
    ...
    # covers [0, 1)
    r = np.asarray(np.random.rand(*shape), dtype=config.floatX)
...
def randint(*shape):
    return np.random.randint(-5, 6, shape)

def randuint(*shape):
    return np.array(np.random.randint(5, size=shape), dtype=np.uint32)
...
def randint_ranged(min, max, shape):
    return np.random.randint(min, max+1, shape)
    ...
    if 'inplace' in kwargs:
        if kwargs['inplace']:
            ...
            if not isinstance(_expected, dict):
                expected = lambda *inputs: np.array(_expected(*inputs),
...
# empty2 case is not supported by np.
...
# c round() fct and np round are not the same!
...
_grad_broadcast_unary_abs1_no_complex = dict(
        normal=[np.asarray(rand_ranged(-1, 1, (2, 3)), dtype=floatX)],
        )

_grad_broadcast_unary_0_2_no_complex = dict(
    # Don't go too close to 2 for tests in float32
        normal=[np.asarray(rand_ranged(0, 1.9, (2, 3)), dtype=floatX)],
        )
...
CeilInplaceTester = makeBroadcastTester(op=inplace.ceil_inplace,
        expected=lambda a: np.asarray(np.ceil(a), a.dtype),
        good=_good_broadcast_unary_normal_no_complex,
...
FloorTester = makeBroadcastTester(op=tensor.floor,
        expected=lambda a: np.asarray(np.floor(a), a.dtype),
        good=_good_broadcast_unary_normal_no_complex,
...
TruncInplaceTester = makeBroadcastTester(
    ...
    expected=lambda a: np.asarray(np.trunc(a), a.dtype),
    good=_good_broadcast_unary_normal_no_complex,
    inplace=True)
...
# see in their file np/lib/function_base.py in class vectorize.__call__
...
AllocTester = makeBroadcastTester(
        ...
        bad_build=dict(
                    ...
                    too_big32=(rand(6, 2, 4), np.
                        int32(6), np.int32(2)),
                    too_big32b=(rand(6, 2, 4), np.
                        int32(6), np.int32(4)),
                    too_big32c=(rand(6, 2, 4), np.
                        int32(2), np.int32(4)),
                    too_big32d=(rand(6, 2, 4), np.
                        int32(2), np.int32(6)),
                    too_big32e=(rand(6, 2, 4), np.
                        int32(4), np.int32(6)),
                    too_big32f=(rand(6, 2, 4), np.
                        int32(4), np.int32(2)),
        ),
)
...
class TestAlloc(unittest.TestCase):
    ...
    def test_alloc_constant_folding(self):
        ...
        for alloc, (subtensor, n_alloc) in zip(self.allocs, [
            ...
            # replaced with ==.
            assert np.sum([isinstance(node.op, type(alloc))
...
def _approx_eq(a, b, eps=1.0e-4):
    ...
    # np.max don't like empty ndarray.
    if a.size == b.size == 0:
        return True
    if np.max(abs_rel_err) >= eps:
        if _approx_eq.debug:
            print(a, b)
        return False
    return True
...
class T_Join_and_Split(unittest.TestCase):
    ...
    def test_stack_new_interface(self):
        """Test the new np-like interface: stack(tensors, axis=0)."""
...
class test_comparison(unittest.TestCase):
    ...
    def test_allclose(self):
        # equal_nan argument not in current version of np allclose,
        # force it to False.
        for dtype in self.dtypes:
            l = np.asarray(
                [0., 1., -1., 0.,
...
class test_matinv(unittest.TestCase):
    ...
    def test_reciprocal(self):
        ...
        utt.seed_rng()
        # hand-coded np implementation for verification
        x = rand(3, 3) + 0.1
        w = rand(3, 3)
        x = np.asarray(x, dtype=config.floatX)
        w = np.asarray(w, dtype=config.floatX)
...
class TestNdGrid(unittest.TestCase):
    ...
    def test_mgrid_theano_variable_np_equiv(self):
    ...
    def test_ogrid_theano_variable_np_equiv(self):
...
class TestPermuteRowElements(unittest.TestCase):
    ...
    def test_2_2(self):
        ...
        # row of input
        out_bis = np.asarray([i_row[p_row] for i_row,
    ...
    def test_3b_2(self):
        ...
        # of the input tensor
        out_bis = np.asarray([[in_mat[0, p_row]
...
def _test_autocast_np():
    """Called from `test_autocast`."""
    assert config.cast_policy == 'np'
    ...
    for x in ([2 ** i for i in xrange(63)] +
              ...
              [0., 1., 1.1, 1.5]):
        n_x = np.asarray(x)
        # Make sure the data type is the same as the one found by np.
        ok(x)
        ok(-x)
        ok(x - 1)
        ok(-x + 1)
                    ...
                    for a_type in dtypes:
                        for b_type in dtypes:
                            ...
                            for combo in (
                                ...
                                np_args = list(map(eval,
                                        ['np_%s' % c for c in combo]))
                                try:
                                    theano_dtype = op(
                                    ...
                                    continue
                                # For np we have a problem:
                                #   http://projects.scipy.org/np/ticket/1827
                                # As a result we only consider the highest data
                                # type that np may return.
                                np_dtypes = [
                                        op(np_args[0](a_type),
                                           np_args[1](b_type)).dtype,
                                        op(np_args[1](b_type),
                                           np_args[0](a_type)).dtype]
                                np_dtype = theano.scalar.upcast(
                                        *list(map(str, np_dtypes)))
                                ...
                                if 'array' in combo and 'scalar' in combo:
                                    ...
                                    # Theano may differ from np as it does
                                ...
                                if (cfg == 'np+floatX' and
                                    ...
                                    np_dtype == 'complex64'):
                                    # In np 1.6.x adding a complex128 with
...
def test_unalign():
    ...
    try:
        ...
        assert not a.flags.aligned
        assert not b.flags.aligned
        assert np.allclose(out_np, out_theano)
        assert False
...
class T_get_scalar_constant_value(unittest.TestCase):
    ...
    def test_np_array(self):
        # Regression test for crash when called on a np array.
...
class test_size(unittest.TestCase):
    """
    Ensure the `size` attribute of tensors behaves as in np.
    """
...
class test_np_assumptions(unittest.TestCase):
    """
...
class TestSpecifyShape(unittest.TestCase):
    ...
    def test_bad_shape(self):
        ...
        for shape in [(1, 3), (2, 2), (5, 5)]:
            xval = np.random.rand(*shape).astype(floatX)
            self.assertRaises(AssertionError, f, xval)
...
class TestTensorInstanceMethods(unittest.TestCase):
    ...
    def test_std(self):
        ...
        # args directly to np. This sometimes results in small
...
class test_ptp(unittest.TestCase):
    def test_scalar(self):
        ...
        self.assertTrue(np.array_equal(result, npResult))
...
class T_Choose(utt.InferShapeTester):
    ...
    def test_np_compare_tuple(self):
...
def test_composite_neg_bool():
    ...
    # `-np.bool_(True)` is False and `-np.bool_(False)` is True.
