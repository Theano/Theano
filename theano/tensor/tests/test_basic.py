np.from __future__ import absolute_import, print_function, division
import itertools
import logging
import operator
import os
...
def randuint16(*shape):
    return np.array(np.random.randint(5, size=shape), dtype=np.uint16)
    ...
    def test0(self):
        ...
        if config.cast_policy == 'custom':
            ...
            self.assertTrue(isinstance(
                v, getattr(numpy, str(np.asarray(56).dtype))))
        else:
...
class test_arithmetic_cast(unittest.TestCase):
    ...
    def test_arithmetic_cast(self):
        ...
        numpy_scalar = lambda dtype: np.array(1, dtype=dtype)
        theano_array = lambda dtype: tensor.vector(dtype=str(dtype))
        numpy_array = lambda dtype: np.array([1], dtype=dtype)
